...
const ElectricityMeasurer = ({ position }) => {
  ...
  useFrame(() => {
    ...
    const rotation = THREE.MathUtils.lerp(
      0, // 270 degrees max rotation
      Math.PI * 1.5, // scale to max 100 kWh
      Math.min(currentReading / 100, 1)
    )
  ...
  return (
    <group ref={measurer} position={position}>
      ...
      {/* Dial */}
      <mesh ref={dialRef} position={[0, 1.3, 0.66]} rotation={[0, 0, 0]}>
