...
import ElectricityMeasurer from "./ElectricityMeasurer";
import ElectricityProducer from "./ElectricityProducer";
import { useAudio } from "@/lib/stores/useAudio";
import { useGameState } from "./useGameState";

const Game = () => {
  const gameRef = useRef<THREE.Group>(null);
  const [waterMolecules, setWaterMolecules] = useState<{ id: number; position: THREE.Vector3 }[]>([]);
  ...
  const collectMolecule = (id: number) => {
    setWaterMolecules(prev => prev.filter(molecule => molecule.id !== id));
    addMolecule(1);
    updateScore(10);
    console.log("Molecule collected! Molecules:", moleculesCollected + 1, "Score:", score + 10);
  };
  
  // Generate electricity from collected water molecules
  const generateElectricity = (amount: number) => {
    addElectricity(amount);
    updateScore(amount * 20);
    console.log("Electricity generated:", amount, "Total:", electricityGenerated + amount);
  };
