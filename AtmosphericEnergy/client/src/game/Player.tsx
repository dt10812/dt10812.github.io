...
const Player = () => {
  ...
  useFrame((_, delta) => {
    ...
    if (isMouseLookEnabled) {
      targetRotationRef.current -= mousePositionRef.current.x * MOUSE_SENSITIVITY;
    ...
    // Limit play area
    positionRef.current.x = THREE.MathUtils.clamp(positionRef.current.x, -15, 15);
    positionRef.current.z = THREE.MathUtils.clamp(positionRef.current.z, -15, 15);
    ...
    cameraOffset.applyAxisAngle(new THREE.Vector3(0, 1, 0), rotationRef.current.y);
