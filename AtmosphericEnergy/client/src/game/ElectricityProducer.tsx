...
interface ElectricityProducerProps {
  position: THREE.Vector3;
  onGenerate: (amount: number) => void;
}

const ElectricityProducer = ({ position, onGenerate }: ElectricityProducerProps) => {
  ...
  useFrame((_, delta) => {
    ...
    if (nearProducer && !isGenerating && cooldown === 0) {
      ...
      setTimeout(() => {
        ...
        if (Math.floor(totalProduced / 20) < Math.floor((totalProduced + amount) / 20)) {
    ...
    <group ref={groupRef} position={[position.x, position.y, position.z]}>
      ...
      {particles.map(particle => (
        <ElectricityParticle 
          ...
          targetPosition={new THREE.Vector3(position.x, position.y + 1.2, position.z)}
        />
      ))}
      ...
      {isGenerating && (
        <mesh position={[0, 2.3, 0]} rotation={[Math.PI / 2, 0, 0]}>
          <ringGeometry args={[0.6, 0.7, 32, 1, 0, Math.PI * 2 * genProgress]} />
